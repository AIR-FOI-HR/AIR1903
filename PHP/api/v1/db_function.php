...
class DB_Functions {
    ...
    public function checkRegisterEmpty($post) {
        if (!isset($post["Ime"]) || ctype_space($post["Ime"]) || empty($post["Ime"]))
            return "Ime";
        if (!isset($post["Prezime"]) || ctype_space($post["Prezime"]) || empty($post["Prezime"]))
            return "Prezime";
        if (!isset($post["Email"]) || ctype_space($post["Email"]) || empty($post["Email"]))
            return "Email";
    ...
    public function storeUser($post, $hash) {
        ...
        $response["Token"] = $this->generateAuth($response["KorisnickoIme"], true);
    ...
    public function checkRegister($post) {
        if (strlen($post["Ime"]) < 3)
            return "Ime";
        if (strlen($post["Prezime"]) < 3)
            return "Prezime";
        if (!preg_match("/(\w+\.)*(\w+)@(\w+\.){1,2}(\w{2,5})/", $post["Email"]))
            return "Email";
        if (strlen($post["KorisnickoIme"]) < 5)
            return "KorisnickoIme";
        if (strlen($post["Lozinka"]) < 7)
            return "Lozinka";
        return 1;
    }
    ...
    public function checkPassword($post) {
        ...
        if ($hash == $hashDB) {
            ...
            $response2["Token"] = $this->generateAuth($response2["KorisnickoIme"], false);
    ...
    public function addNewProduct($post) {
        ...
        if (!isset($_FILES['Slika'])) {
            $q .= "VALUES (null,'{$post["Naziv"]}','{$post["Opis"]}', 'https://cortex.foi.hr/pop/img/defaultPicture.png')";
        } else {
            $slika = $_FILES["Slika"];
            $uploadPath = 'img/'.$event["Id"].'/';
    ...
    public function updateProduct($post) {
		$event = $this->getCurrentEvent();
        if (!isset($_FILES['Slika'])) {
            if ($post["Slika"]=="") $slika = 'https://cortex.foi.hr/pop/img/defaultPicture.png';
    ...
    public function generateAuth($korIme, $short=false) {
        $event = $this->getCurrentEvent();
    ...
    public function addNewPackage($post) {
        ...
        $q = "INSERT INTO Item (Id, Naziv, Opis, Slika, Izbrisan) ";
         if (!isset($_FILES['Slika'])) {
            $q .= "VALUES (null,'{$post["Naziv"]}','{$post["Opis"]}', 'https://cortex.foi.hr/pop/img/defaultPicture.png',0)";
        } else {
            $slika = $_FILES["Slika"];
            $uploadPath = 'img/'.$event["Id"].'/';
            $uploadUrl = '/home/zlatko/public_html/pop/' . $uploadPath;
    ...
    public function checkPackageEmpty($post) {
        if (!isset($post["Naziv"]) || empty($post["Naziv"]) || !isset($post["Popust"]) || empty($post["Popust"]) || !isset($post["Opis"]) || empty($post["Opis"])) {
            return 0;
        } else {
            return 1;
        }
    ...
    public function reduceAmount($post){//CHECKCHECKCHECK
        $racun = $this->getInvoice($post);
    ...
    //funkcija za prikaz racuna prilikom prodaje item-a.
    public function getInvoice($post) {
        ...
        $q = "SELECT r.Id, r.MjestoIzdavanja, r.DatumIzdavanja, r.Popust, r.Id_Trgovine, t.Naziv Trgovina, r.Kupac, k.Ime Ime_Klijenta, k.Prezime Prezime_Klijenta, k.KorisnickoIme"
                    ...
                    . " WHERE r.Id = {$post["Id_Racuna"]}  AND (k.Id_Eventa = {$event["Id"]} OR k.Id_Eventa IS NULL)";//MAYBE
        $stmt = $this->conn->query($q);
        $stmt = $stmt->fetch_assoc();
